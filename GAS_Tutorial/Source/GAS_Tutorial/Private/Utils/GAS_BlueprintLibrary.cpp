...
FClosesActorsWithTagResult UGAS_BlueprintLibrary::FindClosestActorWithTag(UObject* WorldContextObject, const FVector& Origin, const FName& Tag, float SearchRange)
{
	...
	for (auto Actor : ActorsWithTag)
	{
		...
		if (AGAS_BaseCharacter* SearchingCharacter = Cast<AGAS_BaseCharacter>(WorldContextObject))
		{
...
TArray<AActor*> UGAS_BlueprintLibrary::ApplyKnockback(AActor* AvatarActor, const TArray<AActor*>& HitActors, float InnerRadius, float OuterRadius,
                                                      float LaunchForceMagnitude, float RotationAngle, bool bDrawDebugs)
{
	for (AActor* HitActor : HitActors)
	{
		ACharacter* HitCharacter = Cast<ACharacter>(HitActor);
		if (!HitCharacter || !AvatarActor) return TArray<AActor*>();
