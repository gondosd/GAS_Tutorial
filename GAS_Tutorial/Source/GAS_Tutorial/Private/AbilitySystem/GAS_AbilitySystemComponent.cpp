...
{
	if (!GetAvatarActor() || !GetAvatarActor()->HasAuthority()) return;
...
void UGAS_AbilitySystemComponent::AddToAbilityLevel(TSubclassOf<UGameplayAbility> AbilityClass, int32 LevelToAdd)
{
	if (!GetAvatarActor() || !GetAvatarActor()->HasAuthority()) return;
